...
class CombinedModel(nn.Module):
    def __init__(self, num_bins_aesthetic, num_attr, num_bins_attr, num_pt, embed_size=512, num_heads=8):
        ...
        # Linear projection for the combined tensor
        self.combine_project = nn.Linear(embed_size + num_pt, embed_size)

        # For predicting attribute histograms for each attribute
        self.fc_attribute = nn.Sequential(
            nn.Linear(embed_size, embed_size),
            nn.ReLU(),
            nn.Linear(embed_size, num_attr * num_bins_attr) # Each attribute has its own histogram
        )

        # For predicting aesthetic score histogram
        self.fc_aesthetic = nn.Sequential(
            nn.Linear(num_attr * num_bins_attr, embed_size),
            nn.ReLU(),
            nn.Linear(embed_size, num_bins_aesthetic)
        )
    
    def forward(self, images, traits_histogram):
        x = self.resnet(images)

        # Combine features and traits histogram
        combined = torch.cat((x, traits_histogram), dim=1)

        # Project the combined tensor to the embed_size dimension
        projected = self.combine_project(combined)
        
        # Applying self-attention on the projected tensor
        attn_output, _ = self.self_attention(projected.unsqueeze(0), projected.unsqueeze(0), projected.unsqueeze(0))
        attn_output = attn_output.squeeze(0)
        
        attribute_logits = self.fc_attribute(attn_output)
...
if __name__ == '__main__':
    ...
    best_model = None
    best_modelname = 'best_model_resnet50_histo_attr_selfattn_lr%1.0e_decay_%depoch' % (lr, num_epochs)
