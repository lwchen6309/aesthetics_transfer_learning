...
from .gcn_base import GATTP, GATTP_1
...
class Avg_Pool_FC(Graph_Base):
    def __init__(self, in_features=16928):
...
class Avg_Pool_ED(Graph_Base):
    def __init__(self, num_classes, in_features=16928):
...
class GCN_GMP(Graph_Base):
    def __init__(self, num_classes, in_features=16928, hidden=2048, heads=8, out_features=3072, edge_dim=2):
...
class GAT_x3_GATP_MH(Graph_Base):
    ...
    def __init__(self, num_classes, in_features=16928, hidden=2048, heads=8, out_features=3072, edge_dim=2, layers = 3):
        super(GAT_x3_GATP_MH, self).__init__()
        self.layers = layers
        self.lin01 = nn.Sequential(nn.Linear(in_features, hidden, bias=False), nn.LeakyReLU(), \
                                   nn.BatchNorm1d(hidden, eps=0.2))
        self.gnn = nn.ModuleList([Graph_Block(hidden, 128, 16, 0.8) for i in range(self.layers)])
        self.pool = GATTP_1(out_features=hidden, heads=16)
        self.lin2 = nn.Sequential(nn.Dropout(0.8), nn.Linear(hidden, 1024, bias=False), nn.ReLU(), \
                                  nn.BatchNorm1d(1024, eps=0.2))
        self.lin3 = nn.Linear(1024, num_classes)
