...
class Discriminator(nn.Module):
    def __init__(self, num_attr, num_bins_attr, num_bins_aesthetic, num_pt, h_dim=4):
        ...
        self.fc = nn.Sequential(
            ...
            # nn.Linear(num_bins_aesthetic, h_dim),
            nn.ReLU(),
            nn.Linear(h_dim, 1),
        )
    
    def forward(self, aesthetic_logits, attribute_logits):
        # x = aesthetic_logits
...
class CombinedModel(nn.Module):
    def __init__(self, num_bins_aesthetic, num_attr, num_bins_attr, num_pt):
        ...
        self.fc_attribute = nn.Sequential(
            nn.Linear(512, 512),
            nn.ReLU(),
            nn.Linear(512, self.num_attr * self.num_bins_attr)
        )
    ...
    def forward(self, images, traits_histogram):
        ...
        attribute_logits = self.fc_attribute(xz)
        return aesthetic_logits, attribute_logits.view(-1,self.num_attr,self.num_bins_attr)


# Training Function
def train(model, discriminator, dataloader, emd_criterion, optimizer, d_optimizer, device):
    model.train()
    discriminator.train()
    ...
    for sample in progress_bar:
        ...
        total_emd_loss = loss_aesthetic_emd + loss_attribute_emd
        ...
        real_logits = discriminator(aesthetic_score_histogram, attributes_target_histogram)
        ...
        fake_logits = discriminator(prob_aesthetic.detach(), prob_attribute.detach())
        ...
        logits = discriminator(prob_aesthetic, prob_attribute)
...
# Training Function
def train_without_label(model, discriminator, dataloader, optimizer, d_optimizer, device):
    model.train()
    discriminator.train()
    running_aesthetic_emd_loss = 0.0
    running_total_emd_loss = 0.0
    ...
    return epoch_gan_loss, epoch_d_loss
...
resume = 'best_model_resnet50_gan_latefusion_attroutput_lr5e-05_decay_20epoch_genial-morning-423.pth'
resume_D = 'best_model_resnet50_gan_latefusion_attroutput_lr5e-05_decay_20epoch_genial-morning-423_D.pth'
...
def load_data():
    ...
    _, test_user_piaa_dataset = split_dataset_by_user(
        ...
        test_count=40, max_annotations_per_user=[10,50], seed=random_seed)
    ...
    train_piaa_dataset = PARA_PIAA_HistogramDataset(root_dir, transform=test_transform, data=train_dataset.data)
    ...
    return train_dataset, train_piaa_dataset, test_giaa_dataset, test_piaa_dataset, test_user_piaa_dataset
...
if __name__ == '__main__':
    ...
    parser.add_argument("--h_dim", type=int, default=8, help="Hidden dimension of Discriminator")
    parser.add_argument("--num_epochs", type=int, default=20, help="Hidden dimension of Discriminator")
    ...
    if is_log:
        ...
        wandb.init(project="resnet_PARA_PIAA", tags=hyperparam_tags, 
                   notes="Adversial Attr LateFusion AttrOutput")
        experiment_name = wandb.run.name
    else:
        experiment_name = ''
    
    train_dataset, train_piaa_dataset, test_giaa_dataset, test_piaa_dataset, test_user_piaa_dataset = load_data()
    ...
    train_piaa_dataloader = DataLoader(train_piaa_dataset, batch_size=batch_size, shuffle=True, num_workers=n_workers, pin_memory=True, timeout=300)
    ...
    if resume is not None:
        ...
        discriminator.load_state_dict(torch.load(resume_D))

    # Loss and optimizer
    optimizer = optim.Adam(model.parameters(), lr=G_lr)
    d_optimizer = optim.Adam(discriminator.parameters(), lr=D_lr)

    piaa_optimizer = optim.Adam(model.parameters(), lr=G_lr*0.1)
    piaa_d_optimizer = optim.Adam(discriminator.parameters(), lr=D_lr*0.1)

    # Initialize the best test loss and the best model
    best_model = None
    best_modelname = 'best_model_resnet50_gan_latefusion_attroutput_lr%1.0e_decay_%depoch' % (G_lr, num_epochs)
    ...
    for epoch in range(num_epochs):
        ...
        # Training
        # train_piaa_gan_loss, train_piaa_d_loss = train_without_label(model, discriminator, train_piaa_dataloader, piaa_optimizer, piaa_d_optimizer, device)
        # if is_log:
        #     wandb.log({"Train PIAA GAN Loss": train_piaa_gan_loss,
        #             "Train PIAA Discriminator Loss": train_piaa_d_loss,
        #             }, commit=False)
        ...
        if test_giaa_emd_loss < best_test_loss:
            ...
            torch.save(discriminator.state_dict(), best_modelname.replace('.pth', '_D.pth'))
        else:
