...
class Attention(nn.Module):
    def __init__(self, image_feat_dim, trait_dim, attention_dim):
        ...
        # Extract key and value from trait
        self.key_layer = nn.Linear(trait_dim, attention_dim)
        self.value_layer = nn.Linear(trait_dim, attention_dim)
        # Extract query from image feature
        self.query_layer = nn.Linear(image_feat_dim, attention_dim)

    def forward(self, images_feat, traits_histogram):
        keys = self.key_layer(traits_histogram)
        values = self.value_layer(traits_histogram)
        queries = self.query_layer(images_feat)
...
class CombinedModel(nn.Module):
    def __init__(self, num_bins_aesthetic, num_attr, num_bins_attr, num_pt, attention_dim=512):
        ...
        self.proj = nn.Linear(512 + num_pt, attention_dim)
    ...
    def forward(self, images, traits_histogram):
        ...
        attention_output += self.proj(torch.cat((x, traits_histogram), dim=1))
...
if __name__ == '__main__':
    ...
    best_model = None
    best_modelname = 'best_model_resnet50_histo_attr_crossattn_rev_res_lr%1.0e_decay_%depoch' % (lr, num_epochs)
