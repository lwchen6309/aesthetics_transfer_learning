...
def evaluate(model, dataloader, criterion, criterion_raw_ce, criterion_emd, device, num_samples=50):
    model.train()
    running_mse_loss = 0.0
    running_custom_loss = 0.0
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        test_mse_loss, test_mse_std_loss, test_custom_loss, dropout_mean, dropout_std, ce_loss, raw_ce_loss, emd_loss = evaluate(
            model_resnet50, test_dataloader, criterion, criterion_raw_ce, criterion_emd, device, num_samples=5
        )
        if is_log:
            wandb.log({
