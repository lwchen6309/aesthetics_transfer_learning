...
# from train_resnetll_cls_uc import evaluate


def evaluate_ensemble(dataloader, criterion, criterion_raw_ce, criterion_emd, device, ensemble_mean):
    ...
    with torch.no_grad():
        for i, (images, mean_scores, std_scores, score_prob) in enumerate(progress_bar):
            ...
            logit = ensemble_mean[batch_size*i:batch_size*(i+1)]
            ...
            progress_bar.set_postfix({'MSE Loss': mse_mean_loss.item()})
    ...
    return epoch_mse_loss, epoch_mse_std_loss, epoch_ce_loss, epoch_raw_ce_loss, epoch_emd_loss, epoch_brier_score
...
if __name__ == '__main__':
    ...
    for i in range(num_models):
        ...
        best_modelname = 'best_model_resnet50_lluc%d_cls_lr%1.0e_%depoch' % (i, lr, num_epochs)
        if not use_attr:
            best_modelname += '_noattr'
        if not use_uc:
            best_modelname += '_nouc'
        ...
        print(f"Model {i}: Test EMD Loss: {test_emd_loss}, Test Brier Score: {test_brier_score}",
              f"Test CE Loss: {test_ce_loss}, Test Raw CE Loss: {test_raw_ce_loss}",
              f"Test MSE Mean Loss: {test_mse_loss}, Test MSE Std Loss: {test_mse_std_loss}")

        ensemble_mean += dropout_mean
        ensemble_var += (dropout_std ** 2 + dropout_mean**2)
        mean_list.append(dropout_mean)
    ...
    test_mse_loss, test_mse_std_loss, test_ce_loss, test_raw_ce_loss, test_emd_loss, test_brier_score = evaluate_ensemble(
        test_dataloader, criterion, criterion_raw_ce, criterion_emd, device, ensemble_mean)
    
    print(f"Ensemble Model: Test EMD Loss: {test_emd_loss}, Test Brier Score: {test_brier_score}, "
            f"Test CE Loss: {test_ce_loss}, Test Raw CE Loss: {test_raw_ce_loss}, "
            f"Test MSE Mean Loss: {test_mse_loss}, Test MSE Std Loss: {test_mse_std_loss}, "
            )
