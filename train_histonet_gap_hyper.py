...
class HyperConvNetwork(nn.Module):
    def __init__(self, input_dim, ch_in, ch_out, kernel_size=3, hidden_dim=[128, 64, 64]):
        super(HyperConvNetwork, self).__init__()
        
        self.ch_in, self.ch_out, self.kernel_size = ch_in, ch_out, kernel_size
        output_dim = ch_in * ch_out * kernel_size * kernel_size

        self.fc1 = nn.Linear(input_dim, hidden_dim[0])
        self.fc2 = nn.Linear(hidden_dim[0], hidden_dim[1])
        self.fc3 = nn.Linear(hidden_dim[1], output_dim)
        self.bn = nn.BatchNorm2d(ch_out)  # Adding BatchNorm layer
        self.relu = nn.ReLU()
    ...
    def infer_hnet(self, x, trait):
        b = trait.shape[0]
        padding = (self.kernel_size - 1) // 2
        kernel = self.forward(trait).view(b, self.ch_out, self.ch_in, self.kernel_size, self.kernel_size)
        conv_out = torch.stack([F.conv2d(x[i], kernel[i], stride=1, padding=padding) for i in range(b)], dim=0)
        conv_out = self.relu(self.bn(conv_out))
        return conv_out
...
class CombinedModel(nn.Module):
    def __init__(self, num_bins_aesthetic, num_attr, num_bins_attr, num_pt):
        ...
        self.conv_aesthetic = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(512, num_bins_aesthetic, kernel_size=1),
        )

        # HyperNetworks for generating convolution weights
        self.hypernet = [
            HyperConvNetwork(num_pt, ch_in=512, ch_out=512, kernel_size=3).cuda(),
            HyperConvNetwork(num_pt, ch_in=512, ch_out=512, kernel_size=3).cuda(),
            HyperConvNetwork(num_pt, ch_in=512, ch_out=256, kernel_size=3).cuda(),
    ...
    def forward(self, images, traits_histogram):
        ...
        out = self.conv_aesthetic(x)
        # Using HyperNetwork to generate weights for convolutions
        for hnet in self.hypernet:
...
def load_data():
    ...
    # train_dataset = PARA_PIAA_HistogramDataset(root_dir, transform=train_transform, data=train_piaa_dataset.data)
...
if __name__ == '__main__':
    ...
    if is_log:
        ...
        wandb.init(project="resnet_PARA_PIAA", tags=hyperparam_tags, notes="attr_traithyper_gap")
    ...
    best_model = None
    best_modelname = 'best_model_resnet50_histo_traithyper_gap_lr%1.0e_decay_%depoch' % (lr, num_epochs)
